...
# 2. 提交更改
bun run mygit  # 或手动 git add/commit/push
...
`mytag` 命令会自动：
- 读取 `version.json` 中的当前版本号
- 将版本号转换为 Git tag 格式（Ver:1.0.20260206.007 → v1.0.20260206.007）
- 检查未提交的更改和 tag 是否已存在
- 创建带注释的 Git tag
- 推送 tag 到 GitHub
- 触发 GitHub Actions 自动构建

详细说明请查看 [MyTag 使用指南](MYTAG_USAGE.md)。
